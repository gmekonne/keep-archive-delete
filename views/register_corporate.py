...
# =====================================================================
# SECTION 2: CORPORATE ONBOARDING INFORMATION FORM (AUTO-SYNCING ENABLED)
# What it does: Captures user profile fields and forces instant state updates.
# =====================================================================
with st.container(border=True):
    ...
    with col_n1:
        # 🟢 UPGRADE: Added on_change trigger handlers to ensure instant variable collection sync loops
        first_name = st.text_input("First Name *", placeholder="e.g., John", key="corp_fname_widget", on_change=st.rerun)
    with col_n2:
        last_name = st.text_input("Last Name *", placeholder="e.g., Smith", key="corp_lname_widget", on_change=st.rerun)
        
    corp_name = st.text_input("Organization / University Name *", placeholder="e.g., Global Tech University", key="corp_org_widget", on_change=st.rerun)
    corp_email = st.text_input("Administrative Account Email *", placeholder="admin@domain.com", key="corp_email_widget", on_change=st.rerun)
    corp_password = st.text_input("Create Portal Access Password *", type="password", placeholder="Choose a strong password string", key="corp_pass_widget", on_change=st.rerun)
    corp_seats = st.number_input("Target Seat Allocations (Instructor Accounts)", min_value=5, max_value=500, value=25, step=5, key="corp_seats_widget", on_change=st.rerun)
...
if not first_name or not last_name or not corp_name or not corp_email or not corp_password:
    ...
    # Clear the lingering warning message box and compile the custom visual buttons frame
...
paypal_event_payload = streamlit_js_eval(js_script=js_listener_script, key="paypal_bridge_listener_loop_v2")
